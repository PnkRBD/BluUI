local BUI = BluUI
local SetScript = BUI.Prof.Scripts('Pages.Minimap')
...
local function BuildPreview(parent, opts)
    ...
    for _, def in ipairs(ICON_DEFS) do
        ...
        SetScript(proxy, 'OnDragStart', function(self)
            SetScript(self, 'OnUpdate', function(draggedIcon)
        ...
        SetScript(proxy, 'OnDragStop', function(self)
        ...
        SetScript(proxy, 'OnMouseWheel', function(self, delta)
    ...
    local function MakeTextDragger(name, fontString, keyX, keyY, onStop)
        ...
        SetScript(button, 'OnDragStart', function(self)
            local cursorX, cursorY = GetCursorPosition()
            ...
            SetScript(self, 'OnUpdate', function(dragButton)
        ...
        SetScript(button, 'OnDragStop', function(self)
            SetScript(self, 'OnUpdate', nil)
            onStop()
            onLayoutChanged()
        end)
        ...
        SetScript(button, 'OnLeave', function()
            glow:SetVertexColor(1, 1, 1, 0)
            HideTip()
        end)
