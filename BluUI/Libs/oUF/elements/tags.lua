...
local pendingStrings = {}
local pendingCount = 0
local flushScheduled = false

local function flushPendingStrings()
	flushScheduled = false
	local count = pendingCount
	pendingCount = 0
	for i = 1, count do
		local fs = pendingStrings[i]
...
local function queueTagUpdate(fs)
	if(fs.__tagPending) then return end
	fs.__tagPending = true
	pendingCount = pendingCount + 1
	pendingStrings[pendingCount] = fs
	if(not flushScheduled) then
		flushScheduled = true
		C_Timer.After(FLUSH_INTERVAL, flushPendingStrings)
	end
end
...
local function enableTimer(timer)
	...
	if(not frame) then
		...
		frame = CreateFrame('Frame', 'oUF_TagTimer_' .. (timer * 1000))
		frame:SetScript('OnUpdate', function(self, elapsed)
			if(total >= timer) then
				local anyShown = false
				for fs in next, strings do
					if(fs.__owner:IsShown()) then
						anyShown = true
						if(unitExists(fs.__owner.__unit)) then
							fs:UpdateTag()
						end
					end
				end
...
local function Update(self)
	if(STATE[self]) then
		...
		for _, timerFrame in next, timerFrames do
			timerFrame:Show()
		end
	end
end
...
local function outputChanged(fs, format, values, count)
	local seen = fs.__tagSeen
	if(not seen or fs.__tagSeenFormat ~= format) then
		seen = table.wipe(seen or {})
		fs.__tagSeen = seen
		fs.__tagSeenFormat = format
		for i = 1, count do
			seen[i] = values[i]
		end
		return true
	...
	for i = 1, count do
		...
		if(not dirty) then
			local prev = seen[i]
			if(prev == nil or issecretvalue(value) or issecretvalue(prev) or value ~= prev) then
				dirty = true
			end
		end
		seen[i] = value
...
local function getTagFunc(tagstr)
	...
	if(not func) then
		...
		func = function(self)
			...
			if(outputChanged(self, format, buffer, num)) then
...
oUF.Tags = {
	...
	SetEventUpdateTimer = function(self, timer)
		...
		FLUSH_INTERVAL = math.max(0.05, timer)
	end,
}
